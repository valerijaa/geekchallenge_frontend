            ...
            <div class="inner-content">
              <div class="components"> 
                <div class="language">
                  ...
                  <ul> 
                    <li v-bind:key="toneResult.tone_id" v-for="toneResult in post.analysis_result.emotion.scores">
                      <span style="color:goldenrod;font-weight:bold;margin-right: 10px;">{{getPercentage(toneResult.score)}} % </span> {{toneResult.tone_name}} 
                    </li>
                  </ul>
                </div>
                <div class="emotional">
                  <h2>Language tone</h2>
                  <ul> 
                     <li v-bind:key="toneResult.tone_id" v-for="toneResult in post.analysis_result.language.scores">
                      <span style="color:goldenrod;font-weight:bold;margin-right: 10px;">{{getPercentage(toneResult.score)}} % </span> {{toneResult.tone_name}} 
                    </li>
                  </ul>
                </div>
                <div class="social">
                  <h2>Social tone</h2>
                    <ul> 
                      <li v-bind:key="toneResult.tone_id" v-for="toneResult in post.analysis_result.social.scores">
                        <span style="color:goldenrod;font-weight:bold;margin-right: 10px;">{{getPercentage(toneResult.score)}} % </span> 
                        <template v-if="toneResult.tone_id === 'conscientiousness_big5'">
                          Scrupulosity
                        </template>
    ...
    methods: {
      ...
      toggleVisibility(postId){
        if (this.isCardExpanded(postId)){
          this.expandedPostIds = this.expandedPostIds.filter(expandedPostId => expandedPostId !== postId);
        } else {
